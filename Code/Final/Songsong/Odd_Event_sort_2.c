...
void OddEvenIter(int locale_A[],int temp_B[],int temp_C[], int locale_n,int phase,
        int even_partner, int odd_partner, int rank,int p) {
...
void Sort(int local_A[],int locale_n,int rank,int p) {
    ...
    for(phase = 0;phase < p;phase++) {
        OddEvenIter(local_A,temp_B,temp_C,locale_n,phase,
                                even_partner,odd_partner,rank,p);
    }
    free(temp_B);
    free(temp_C);
}
...
int main(int argc, char** argv) {
    ...
    Sort(localeData,localeSize,rank,comm_size);
